...
export default async function handler(req, res) {
    ...
    if (!id || !email) return res.status(400).json({ success: false, message: 'ID dan Email wajib ada' });
        ...
        if (result.success && result.data.status === 'success') {
            ...
            if (user) {
                ...
                if (!alreadyDone) {
                    ...
                    res.setHeader('Access-Control-Allow-Origin', '*');
                    return res.status(200).json({ success: true, status: 'success', added: amountToAdd });
                }
            }
        }
